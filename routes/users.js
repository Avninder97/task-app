...
const { createIfNotExistUsersTable, createIfNotExistTokensTable } = require('../utils/dbHelper');
const { encryptor, createToken, verifier } = require('../utils/auth');
const { tokenValidator } = require('../middlewares/auth');


router.get('/users/me', tokenValidator, async (req, res) => {
    try {
...
router.post('/users/logout', tokenValidator, async (req, res) => {
    try {
...
router.post('/users/logoutAll', tokenValidator, async (req, res) => {
    try{
        const id = req.decoded.id;
        const deleteResult = await pool.query(
...
router.patch('/users/me', tokenValidator, async (req, res) => {
    try {
...
router.delete('/users/me', tokenValidator, async (req, res) => {
    try {
        const id = req.decoded.id;
